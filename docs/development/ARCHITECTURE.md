...
This document describes the technical architecture of the Ksenia Lares custom component for Home Assistant.

## Directory Structure

```text
custom_components/ksenia/
...
│   ├── __init__.py          # Exports KseniaLaresDataUpdateCoordinator
...
│   ├── __init__.py          # Exports KseniaLaresEntity
...
**Key class:** `KseniaLaresDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `KseniaLaresApiClient`
...
- `KseniaLaresConfigFlowHandler` (main flow)
- `KseniaLaresOptionsFlow` (options)
...
**Key class:** `KseniaLaresEntity` (in `entity/base.py`)
...
2. `KseniaLaresEntity` for common functionality
...
1. Create directory: `custom_components/ksenia/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `KseniaLaresEntity`
